...
```bash
npm install
npm run migrate:extract              # source HTML -> source-data.json
npm run migrate:seed -- --dry-run    # review the plan (no connection opened)
npm run migrate:seed                 # create indexes, load 58 docs
```

Then, once every person's export has been collected (see below):
